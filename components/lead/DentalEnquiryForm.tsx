// DEP-34 — the dental enquiry form. One component, mounted on several pages.
//
...
// the 409 branch and not the error branch. Nursing, pharmacy and AHS followed the same day.
// This is the same shape, pointed at the dental institution.
//
...
import { submitDentalEnquiry, type DentalEnquiryState } from "@/app/actions/dental-enquiry"
import { DENTAL_PROGRAMMES } from "@/lib/dental-programmes"

const ADMISSION_PHONE = "+919345855001"
const ADMISSION_PHONE_DISPLAY = "+91 93458 55001"

type Props = {
  /** Which page this instance sits on, e.g. "bds". Tags the lead in the CRM. */
  sourcePage: string
...
export default function DentalEnquiryForm({
  sourcePage,
  defaultProgrammeId,
  heading = "Get a Call Back",
}: Props) {
  const [state, formAction, isPending] = useActionState<DentalEnquiryState | null, FormData>(
    submitDentalEnquiry,
    null
  )
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="dn-name" className="block text-white text-sm font-medium mb-1.5">
        ...
        <div>
          <label htmlFor="dn-phone" className="block text-white text-sm font-medium mb-1.5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="dn-programme" className="block text-white text-sm font-medium mb-1.5">
        ...
        <div>
          <label htmlFor="dn-district" className="block text-white text-sm font-medium mb-1.5">
      ...
      <div>
        <label htmlFor="dn-question" className="block text-white text-sm font-medium mb-1.5">
        ...
        <textarea
          ...
          rows={3}
          maxLength={300}
          placeholder="e.g., What is the fee for BDS?"
