// DEP-34 — server action behind the dental enquiry form.
//
...
import {
  DENTAL_INSTITUTION_ID,
  VALID_DENTAL_PROGRAMME_IDS,
} from "@/lib/dental-programmes"

const CRM_SLUG = "jkkn-admission-2026"
const CRM_SUBMIT = `https://www.jkkn.ai/api/public/forms/${CRM_SLUG}/submit`

export type DentalEnquiryState = {
  success: boolean
  message?: string
  error?: string
...
function validate(name: string, phone: string, programme: string) {
  const fieldErrors: DentalEnquiryState["fieldErrors"] = {}
  ...
  if (!VALID_DENTAL_PROGRAMME_IDS.has(programme))
    fieldErrors.programme = "Please choose a programme."
  return fieldErrors
}

export async function submitDentalEnquiry(
  _prevState: DentalEnquiryState | null,
  formData: FormData
): Promise<DentalEnquiryState> {
  ...
  const payload = {
    formData: {
      ...
      district,
      institution_program: {
        institution_id: DENTAL_INSTITUTION_ID,
    ...
    utmSource: "dental.jkkn.ac.in",
    utmMedium: "site-form",
    utmCampaign: sourcePage ? `dental-${sourcePage}` : "dental-site",
    campaignLinkId: null,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[dental-enquiry] CRM rejected submit", res.status, body)
      return {
        success: false,
        error:
    ...
    console.error("[dental-enquiry] network/timeout reaching CRM", err)
    return {
      success: false,
      error:
